...
namespace BusinessObject.Attributes
{
    public class UniqueDateCreateShow : ValidationAttribute
    {
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            dynamic objectInstance = validationContext.ObjectInstance;
            int? filmId = (int)objectInstance.FilmId;
            int? roomId = (int)objectInstance.RoomId;
            if (filmId == null || roomId == null) return new ValidationResult("Select Film and Room before choose show date.");
            ...
            if (DateTime.Compare(date, film.DateRelease) < 0) return new ValidationResult($"Choose date greater {film.DateRelease.ToString("dd/MM/yyyy hh:mm:ss")}.");
